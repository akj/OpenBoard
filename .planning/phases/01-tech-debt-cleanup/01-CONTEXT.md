...
- **D-21:** `Downloader.download_file` (`openboard/engine/downloader.py`) gains:
  ...
  - **SHA-256 verification**: when downloading Stockfish releases, fetch the corresponding `.sha256` (or `.checksums`) file from the same GitHub release, verify the downloaded archive's hash before extracting, and fail with `DownloadError` on mismatch. If GitHub does not publish per-asset checksums for the Stockfish release in question, document the gap with a TODO and fall back to HTTPS-only with a clearly-logged WARNING (this is acceptable v1 scope; full release-signing is a v1.x concern). **CLARIFICATION (post-RESEARCH.md Pitfall 5):** Stockfish releases publish NO per-asset checksum files, so the WARNING fallback IS the primary path in v1. The `expected_sha256` parameter to `download_file` is reserved for callers who already have a hash (test fixtures, future release-signing); production calls from `StockfishManager` pass `expected_sha256=None` and rely on HTTPS + WARNING. SHA-256 verification code is still implemented for the parameterized path.
